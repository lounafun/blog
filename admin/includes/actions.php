...
$logins = array(
	array('username' => 'sev', 'password'=> 'babareactor07'),
...
if (isset($_POST['action']) && $_POST['action'] === 'add_article'){
	...
	if (count($messages) === 0) {
		mysql_query("INSERT INTO articles (title, text, date) VALUES 
		('" . mysql_real_escape_string($title) . "', '" . mysql_real_escape_string($text) ."', NOW())");
